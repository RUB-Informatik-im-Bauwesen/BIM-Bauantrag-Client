...
public class Sonderbau extends ExtraktionsVorgang {
	
	private int sonderbau;
	
	public Sonderbau() {
		this.name="Sonderbau nach § 2 Abs. 4 MBO";
	}
	...
	public void perform(ApplicationModelNode ifcModel, XPlanungModel xPlanModel) {
		if (ifcModel != null) {
			...
			if (project instanceof IfcProject.Ifc4) {
				IfcSimpleProperty.Ifc4 simpleProperty = Util.Ifc4.getElementProperty((IfcObjectDefinition.Ifc4) project, "BauantragAllgemein", "Sonderbau");
				...
				if (enumeratedValue.getEnumerationValues()!=null && enumeratedValue.getEnumerationValues().size() > 0) {
					...
					if(value instanceof IfcPositiveInteger) {
						sonderbau = ((IfcPositiveInteger.Ifc4)value).getValue();
					}else if(value instanceof IfcInteger) {
						sonderbau = ((IfcInteger.Ifc4)value).getValue();
					}else if (value instanceof IfcLabel) {
						sonderbau=Integer.valueOf(((IfcLabel.Ifc4)value).getDecodedValue());
					}
					
					this.value = Integer.toString(sonderbau);
