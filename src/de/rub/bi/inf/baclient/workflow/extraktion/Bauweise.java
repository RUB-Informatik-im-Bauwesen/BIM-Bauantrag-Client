...
public class Bauweise extends ExtraktionsVorgang {
	
	public Bauweise() {
		this.name="Bauweise";
	}
	
	private int bauweise;
	...
	public void perform(ApplicationModelNode ifcModel, XPlanungModel xPlanModel) {
		if (ifcModel != null) {
			...
			if (project instanceof IfcProject.Ifc4) {
				IfcSimpleProperty.Ifc4 simpleProperty = Util.Ifc4.getElementProperty((IfcObjectDefinition.Ifc4) project, "BauantragAllgemein", "Bauweise");
				...
				if (enumeratedValue.getEnumerationValues()!=null && enumeratedValue.getEnumerationValues().size() > 0) {
					...
					if(value instanceof IfcPositiveInteger) {
						bauweise = ((IfcPositiveInteger.Ifc4)value).getValue();
					}else if(value instanceof IfcInteger) {
						bauweise = ((IfcInteger.Ifc4)value).getValue();
					}else if (value instanceof IfcLabel) {
						bauweise=Integer.valueOf(((IfcLabel.Ifc4)value).getDecodedValue());
					}
					
					this.value = Integer.toString(bauweise);
			...
			if(project instanceof IfcProject.Ifc2x3) {
				...
				IfcSimpleProperty.Ifc2x3 simpleProperty = Util.Ifc2x3.getElementProperty((IfcObject.Ifc2x3) project, "BauantragAllgemein", "Bauweise");
